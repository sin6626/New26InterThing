  ...
  it('maps database rows and uses the same filters for count and list', async () => {
    const query = vi.fn()
      ...
      .mockResolvedValueOnce([[
        { id: 7, d_no: '202111', e_no: 'E001', type: '3', e_msg: '传感器故障', c_time: '2026-09-11 10:00:00' },
      ]])
    const repository = createFaultRepository({ query } as never)
    const input = {
      page: 2, pageSize: 10, deviceNumber: '202111', type: '3',
    ...
    await expect(repository.list(input)).resolves.toEqual({
      total: 1,
      items: [{
        id: 7, deviceNumber: '202111', errorNumber: 'E001', type: '3', message: '传感器故障', occurredAt: '2026-09-11 10:00:00',
      }],
    })
    expect(query.mock.calls[0][0]).toContain('d_no = ?')
    expect(query.mock.calls[0][0]).toContain('type = ?')
    expect(query.mock.calls[0][1]).toEqual(['202111', '3', '2026-09-10 08:00:00', '2026-09-11 10:00:00'])
    expect(query.mock.calls[1][1]).toEqual(['202111', '3', '2026-09-10 08:00:00', '2026-09-11 10:00:00', 10, 10])
  })
