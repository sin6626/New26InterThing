...
后端重新读取配置、校验值和安全边界。只有 `master`、`pump`、`heater` 三个运行指令按配置生成 MQTT 报文；目标温度、PID、超时和安全阈值等控制参数只保存到 `t_direct_global`，供状态机判断，不发送 MQTT。MQTT 使用 QoS 1 单次发布，不离线排队、不重试。成功只代表 Broker 已确认接收；发布成功后才更新状态并写成功日志。发布失败不修改当前状态，但会写失败日志。
